다음 순서대로 작업을 진행해주세요.
주어진 페이지가 여러개인 경우, 먼저 모든 페이지에 대한 Step 1, 2를 병렬로 수행하고,
각 페이지에 대해 이후 단계를 반복 수행합니다.:
모든 작업을 마칠 때까지 절대 중단하지 마세요.
...
`implement` agent를 실행하여 Step 4에서 작성된 최종 보고서(`./agent-outputs/$2/3-implement-plan-final.md`)를 정확히 구현합니다.
...
추가된 모든 보고서와, 변경된 코드들을 적절한 메세지로 commit 합니다.
